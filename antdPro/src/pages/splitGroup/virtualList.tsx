...
export const VirtualList = (props: IVirtualList) => {
  ...
  const { onCollapseChange, collapseKeys } = props;
  ...
  const init = () => {
    ...
    let showNum = Math.ceil(height / presetItemHeight) + 1;
    if(showNum >= total){
      showNum = total - 1
    }
    show_num = showNum;
  ...
  const bindHandleScroll = (event:any) => {
    ...
    if (num < total) {
      setStartIndex(Math.max(num - 1, 0));
  ...
  return (
    <>
      <div ref={virtualRef} id="virtual-scroll" className="virtual-scroll-box">
        ...
        >
          <Collapse
            onChange={(e) => onCollapseChange(e)}
            activeKey={collapseKeys} 
            expandIconPosition='end'
            className='site-collapse-custom-collapse'
          >
            {Array.from({ length: endIndex - startIndex }).map((_v, i) => {
              return props.itemRender(startIndex + i)
            })}
          </Collapse>
        </div>
      </div>
